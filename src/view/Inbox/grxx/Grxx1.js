...
import touch_select from '@/view/components/touch-select';
import city_data from '@/view/components/touch-select/data'; //这是插件内置的省市区数据 data.js 修改 或者自行调用接口获取
import "@/view/components/touch-select/touch-css.css";

@connect(store =>({store:store}),AboutAction)
@withRouter
class Grxx1 extends Component {
    state={
        time1:123
    };
    //监听内容更新
    componentWillUpdate(props,state){
        if(this.state!==state){
            console.log(this.state+'更新前的state')
            console.log(state+'更新后的state')
        }
    }
    //props修改钩子
    ...
    render() {
        return (
            <div>
                ...
                <h3>Grxx1</h3>
                <div>{this.state.time1}</div>
                <Button onClick={ ()=>this.handelclick('/Inbox/Grxx2') }>点击跳转到Grxx2</Button>
                <Button onClick={ ()=>this.handelclick('/About') }>点击跳转到About</Button>
                <Button onClick={ ()=>this.changestore('/About') }>修改store</Button>
